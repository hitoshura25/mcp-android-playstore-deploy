...
@mcp.tool()
async def analyze_android_project(project_path: str) -> str:
    ...
    """
    result = generator.analyze_android_project(project_path=project_path)
...
async def validate_github_secrets(
    repo_owner: str, repo_name: str, github_token: str, required_secrets: str = None
) -> str:
...
@mcp.tool()
async def create_github_secrets_guide(repo_url: str, keystore_path: str = None) -> str:
...
async def validate_play_store_setup(service_account_json_path: str, package_name: str) -> str:
    ...
    result = generator.validate_play_store_setup(
        service_account_json_path=service_account_json_path, package_name=package_name
    )
    # Handle both sync and async business logic
    if inspect.isawaitable(result):
        result = await result
